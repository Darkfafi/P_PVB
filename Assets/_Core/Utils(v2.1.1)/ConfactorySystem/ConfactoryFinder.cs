...
namespace Ramses.Confactory
{
	...
	public class ConfactoryFinder : IConfactoryFinder
	{
		...
		public T Get<T>() where T : IConfactory
		{
		...
		private IConfactory CreateConfectory(Type t)
		{
			...
			Type confectoryType = confectory.GetType();

			confectory = GetRepresentingConfactory(confectory);

			activeConfectories.Add(confectoryType, confectory);
			return confectory;
		}
		...
		private void OnSceneSwitchedEvent(string previousScene, string currentScene)
		{
			foreach(KeyValuePair<Type, IConfactory> confactoryPair in activeConfectories)
			{
				IConSceneSwitchUser sceneSwitchUser = confactoryPair.Value as IConSceneSwitchUser;
				if (sceneSwitchUser != null)
				{
					sceneSwitchUser.OnConSceneSwitched(previousScene, currentScene);
				}
            }
		}
    }
