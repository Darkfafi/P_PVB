...
namespace Ramses.Entities
{
	...
	/// </summary>
	public class ConEntityDatabase : IConfactory, Confactory.Addons.IConStructUser
	{
        ...
        private ConEntityIdHandler conEntityIdHandler;


        public void ConClear() {
            conEntityIdHandler = null;
        }

        public IConfactory ConStruct(IConfactoryFinder confactoryFinder) {

            conEntityIdHandler = confactoryFinder.Get<ConEntityIdHandler>();
            return null;
        }
		...
		public void UnRegisterEntity(IEntity entity)
		{
			if (entity != null)
			{
				if (allEntities.Contains(entity))
				{
                    ...
                    if(CheckNetworkIdReset()) {
                        conEntityIdHandler.ResetSpawnCount();
                    }
        ...
        public INetworkedEntity GetNetworkedEntityWithId(int id) {
            INetworkedEntity current = null;
            for (int i = 0; i < allEntities.Count; i++) {
                current = (INetworkedEntity)allEntities[i];
                if (current != null && current.GetUniqueId() == id) {
                    return current;
                }
            }
            return null;
        ...
        private bool CheckNetworkIdReset() 
        {
            bool reset = allEntities.Count == 0;

            if (!reset) {
                for (int i = 0; i < allEntities.Count; i++) {
                    if (((INetworkedEntity)allEntities[i]) != null) {
                        break;
                    }
