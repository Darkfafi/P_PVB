...
public class PlatformerMovement2D {
	...
	public Vector2 Velocity { get { return rbody2D.velocity; } }
	...
	public void Move(int horizontalDirection, float speedMultiplier = 1, bool useVelocityMovement = true)
	{
        ...
        if (touchDetector2D.GetHitInfoFromSide(new Vector2(horizontalDirection, 0)) == null) //Don't move if something is blocking your path
		{
			...
			float finalCalc = (horizontalDirection * calculatedSpeed);
			if (useVelocityMovement)
			{
				StopMoveVelocity();
				rbody2D.velocity = new Vector2(finalCalc, rbody2D.velocity.y);
			}
			else
			{
				finalCalc *= Mathf.Abs(transformObject.localScale.x) / transformObject.localScale.x;
				transformObject.Translate(new Vector2(finalCalc, 0) * Time.deltaTime);
			}
